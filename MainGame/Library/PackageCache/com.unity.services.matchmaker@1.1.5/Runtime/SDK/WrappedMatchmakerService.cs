...
namespace Unity.Services.Matchmaker
{
    internal class WrappedMatchmakerService : IMatchmakerSdkConfiguration, IMatchmakerService
    {
        ...
        internal WrappedMatchmakerService(IMatchmakerServiceSdk matchmakerService, IPayloadProxyApiClient payloadProxyClient = null, IABRemoteConfig abRemoteConfig = null, IABAnalytics abAnalytics = null, IEnvironmentId environmentIdComponent = null, ICloudProjectId projectIdComponent = null)
        {
            ...
            projectIdComponent ??= CoreRegistry.Instance.GetServiceComponent<ICloudProjectId>();
            var projectId = projectIdComponent?.GetCloudProjectId() ?? "";

            environmentIdComponent ??= CoreRegistry.Instance.GetServiceComponent<IEnvironmentId>();
            var environmentId = environmentIdComponent?.EnvironmentId ?? "";
            ...
            if (abAnalytics == null)
            {
                var softAnalytics = CoreRegistry.Instance.GetServiceComponent<IAnalyticsStandardEventComponent>();
                m_abAnalytics = new ABAnalytics(environmentId, softAnalytics);
            }
            else
            {
        ...
        public async Task<BackfillTicket> ApproveBackfillTicketAsync(string backfillTicketId)
        {
            ...
            var config = Configuration.MergeConfigurations(
                new Configuration(
                    null,
                    5,  // Approve is meant to be called every seconds
                    null,
                    null),
                m_MatchmakerService.Configuration);
            var request = new ApproveBackfillTicketRequest(backfillTicketId);
            var response = await TryCatchRequest(m_MatchmakerService.BackfillApi.ApproveBackfillTicketAsync, request, config);
        ...
        private async Task<Response<TReturn>> TryCatchRequest<TRequest, TReturn>(Func<TRequest, string, Configuration, Task<Response<TReturn>>> func, TRequest request, Configuration configuration = null)
        {
            Response<TReturn> response = null;
            try
            {
                response = await func(request, m_payloadProxyToken, configuration ?? m_MatchmakerService.Configuration);
            }
            catch (HttpException he)
            {
                int httpErrorStatusCode = (int)he.Response.StatusCode;
